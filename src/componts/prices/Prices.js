...
import { addingCarPrices, addPrice, setCheckboxesState, deletePrice, handleChange } from './handleFunctions';
    ...
    let routesCB = useMemo(() => {
        ...
        return (
            routes.map((route, index) => {
                return (
                    <PricesInput input={route} inputsArray={routes} index={index} classes={classes} thisPrices={thisPrices} handleChange={addingCarPrices} prices={prices} currentComponent={thisPrices} />
                )
            }))
    ...
    let pricesCB = useMemo(() => {
        ...
        return (
            pricesForm.map((priceInput, index) => {
                return (
                    <PricesInput input={priceInput} inputsArray={pricesForm} index={index} classes={classes} thisPrices={thisPrices} handleChange={addingCarPrices} prices={prices} currentComponent={thisPrices} />
                )
            })
        )
            ...
            <div className="Prices" style={{ textAlign: "left", width: "100%" }}>
                ...
                <div>
                    {prices.length > 1 && index !== 0 ?
                        <Delete onClick={() => deletePrice(prices, changePrices, index, setCheckboxesState, changeCheckboxes, carFromeOut, checkboxes, deletePrice)} />
                            ...
                            return (
                                <>
                                    {checkbox.car.inputs[0].value || checkbox.car.inputs[1].value ?
                                        ...
                                        <PricesCheckbox index={index} car={checkbox.car} classes={classes} thisPrices={thisPrices} checkbox={checkbox} changeCars={changeCars} cars={cars} handleChange={handleChange} />
